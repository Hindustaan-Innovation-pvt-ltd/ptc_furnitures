            ...
            void (async () => {
              try {
                ...
                if (!versionDoc || versionDoc.version !== "5.1") {
                  ...
                  const { rewatermarkImage } = await import(
                    "./image-processor"
                  );

                  const products = await Product.find({});
                  console.log(
                    `==> [Sync] Found ${products.length} products to synchronize...`,
                  );
                  ...
                  for (const prod of products) {
                    ...
                    for (const img of originalImages) {
                      const rewatermarked = await rewatermarkImage(
                        img,
                        prod.brand,
                      );
                    ...
                    if (count % 20 === 0 || count === products.length) {
                      console.log(
                        `==> [Sync] Synchronized ${count}/${products.length} products...`,
                      );
                    }
                  }
                  ...
                  console.log(
                    "==> [Sync] Successfully synchronized all watermarks in MongoDB to fixed 200px size!",
                  );
                } else {
                  console.log(
                ...
                console.error(
                  "==> [Sync] Failed to synchronize watermarks in background:",
                  err.message,
                );
              }
            })();
