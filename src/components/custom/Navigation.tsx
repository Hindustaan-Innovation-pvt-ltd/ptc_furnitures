        ...
        <header className="border-b border-slate-200/80 dark:border-white/10 bg-white/70 dark:bg-[#08090d]/80 backdrop-blur supports-backdrop-filter:bg-white/60 dark:supports-backdrop-filter:bg-[#08090d]/70 transition-colors duration-300">
            <nav className="relative mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
                ...
                <div className="flex items-center gap-2">
                    <button className="lg:hidden rounded-full p-2 text-slate-700 transition hover:bg-slate-100 dark:text-slate-200 dark:hover:bg-white/10" aria-label="menu" onClick={() => setOpen((v) => !v)}>
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M4 6H20M4 12H20M4 18H20" stroke="#2D264B" strokeWidth="1.5" strokeLinecap="round" />
                        </svg>
                    </button>
                    <div aria-label='icons' className="hidden sm:flex items-center gap-2">
                        <div className="relative w-11 overflow-hidden rounded-full p-3 transition-[width,background-color,border-color,box-shadow] duration-300 ease-out hover:w-56 focus-within:w-56 hover:border-slate-300 hover:border focus-within:border-slate-300 focus-within:shadow-lg focus-within:shadow-black/10 dark:border-stone-700/90 dark:hover:border-stone-500 dark:focus-within:border-stone-500 motion-reduce:transition-none xl:w-12 xl:hover:w-60 xl:focus-within:w-60">
                            <svg width="24" height="24" className="size-4 text-slate-200" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
            ...
            {open && (
                <div className="md:hidden absolute left-0 right-0 top-full border-b border-slate-200/80 dark:border-white/10 bg-white dark:bg-[#0c0d11] shadow-lg p-4">
