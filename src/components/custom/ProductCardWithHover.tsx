...
export default function ProductCardWithHover({ product, priority = false }: ProductCardWithHoverProps) {
    ...
    const pendingDownload = React.useRef<{ imageUrl: string; index: number | null } | null>(null);
    ...
    async function handleLeadConfirmed(lead: { name: string; mobile: string }) {
        ...
        fetch("/api/download-leads", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ name: lead.name, mobile: lead.mobile, action: "image_download", productId: product.id, productName: product.name ?? product.brand }),
        }).catch(() => { });
        ...
        <>
            ...
            <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
                            ...
                            <div className="overflow-hidden rounded-xl aspect-square flex items-center justify-center bg-slate-50 relative">
                                {product.images && product.images.length > 1 ? (
                                    <Carousel className="w-full aspect-square relative group/carousel" opts={{ loop: true }}>
                                        ...
                                        <CarouselPrevious onClick={(e) => e.stopPropagation()} className="absolute left-2 top-1/2 -translate-y-1/2 opacity-0 group-hover/carousel:opacity-100 transition-opacity duration-300 bg-white/90 hover:bg-white border border-slate-200 size-7 shadow-xs text-slate-800" />
                                    ...
                                    <Tooltip>
                                        <TooltipTrigger asChild>
                                            <Button
                                                size="icon"
                                                variant="outline"
                                                onClick={() => requestDownload(product.images?.[0] ?? "", null)}
                ...
                <DialogContent className="max-w-[calc(100%-2rem)] sm:max-w-3xl rounded-3xl p-6 pt-12 bg-white border border-slate-200 text-slate-900 shadow-2xl overflow-hidden dark:bg-white dark:text-slate-900 dark:border-slate-200">
                    ...
                    <DialogDescription className="sr-only">Detailed view of {product.name} including reviews, images, and brand details.</DialogDescription>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start max-h-[80vh] overflow-y-auto pr-1">
                        {/* Left Column: Images */}
                        <div className="flex flex-col gap-4">
                            <div className="flex items-center justify-between border-b border-slate-100 pb-2">
                                <span className="text-xs uppercase tracking-wider font-extrabold text-slate-400">Image Views</span>
                                <div className="flex items-center gap-0.5 bg-slate-100 p-0.5 rounded-full border border-slate-200/50">
                                    <button
                                        onClick={() => setViewMode("carousel")}
                                ...
                                {viewMode === "carousel" ? (
                                    product.images && product.images.length > 1 ? (
                                        <Carousel className="w-full h-full relative" opts={{ loop: true }}>
                                            <CarouselContent className="ml-0 h-full">
                                                {product.images.map((image, index) => (
                                                    <CarouselItem key={`dialog-${product.id}-img-${index}`} className="pl-0 h-full flex items-center justify-center">
                                                        <AssetImage
                                                            brand={product.brand}
                                                            src={image}
                                                            alt="Dialog Preview"
                                        ...
                                        <AssetImage brand={product.brand} src={product.images?.[0] ?? ""} alt="Dialog Preview" width={300} height={300} className="size-full object-contain max-h-72" />
                                    )
                                ) : (
                                    <div className="grid grid-cols-3 gap-2 size-full overflow-y-auto pr-0.5 scrollbar-thin">
                                        {product.images && product.images.length > 0 ? (
                                            product.images.map((image, idx) => (
                                                <div key={idx} className="relative aspect-square border border-slate-100 rounded-xl overflow-hidden bg-slate-50 flex items-center justify-center group/dialog-thumb hover:border-red-500/30 transition-all duration-300">
                                                    <AssetImage
                                                        brand={product.brand}
                                                        src={image}
                                                        alt="Grid preview"
                        ...
                        <div className="flex flex-col h-full gap-4">
                            ...
                            <div className="flex items-start justify-between gap-3 border-b border-slate-100 pb-3">
                                <div className="min-w-0">
                                    <h4 className="text-lg font-black leading-tight text-slate-950 truncate">{product.name}</h4>
                                    <span className="text-[10px] font-extrabold tracking-widest uppercase text-red-600 block mt-1">{product.brand || "Exclusive"}</span>
                                </div>
                                <div className="flex items-center gap-3 shrink-0">
                                    <div className="flex flex-col items-end gap-1">
                                        <RatingStars rating={averageRating} size={12} />
                                        <span className="text-xs font-bold text-slate-400">{reviews.length} review{reviews.length === 1 ? "" : "s"}</span>
                                    </div>
                                    ...
                                    <TooltipProvider>
                                        <Tooltip>
                                            ...
                                            <TooltipContent side="top" className="px-2 py-1 rounded font-bold text-[9px] z-50 bg-slate-950 text-white">
                                                <span>Download Watermarked PNG</span>
                                            </TooltipContent>
                                        </Tooltip>
                                    </TooltipProvider>
                            ...
                            <div className="flex flex-col gap-2">
                                <span className="text-xs uppercase tracking-wider font-extrabold text-slate-400">Reviews ({reviews.length})</span>
                                <div className="max-h-48 overflow-y-auto pr-0.5 flex flex-col gap-2 scrollbar-thin">
                                    {reviews.length === 0 ? (
                                        <span className="text-xs text-slate-400 italic">No reviews yet. Be the first to leave a review!</span>
                                    ) : (
                                        reviews.map((rev, index) => (
                                            <div key={index} className="bg-slate-50 border border-slate-100/50 rounded-xl p-3 text-xs leading-relaxed">
                            ...
                            <form onSubmit={handleAddReview} className="mt-auto pt-3 border-t border-slate-100 flex flex-col gap-3">
                                <div className="flex items-center justify-between">
                                    <span className="text-xs uppercase tracking-wider font-extrabold text-slate-400">Add a Review</span>
