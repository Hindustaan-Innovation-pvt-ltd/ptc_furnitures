...
export default async function CatalogDetailsPage({
  ...
  return (
    <Suspense fallback={<CatalogDetailsPlaceholder />}>
      <CatalogDetailsLoader params={params} />
    </Suspense>
  );
}
        ...
        {isPdf ? (
              ...
              >
                <div className="flex flex-col items-center justify-center h-full p-12 text-center">
                  ...
                  <p className="text-xs text-slate-400 mt-1 mb-6">
                    Your browser does not support embedded PDF viewer.
                  </p>
                  <CatalogDownloadButton
                    href={catalog.pdfUrl!}
                    isPrint={false}
          ...
          <div className="print-container max-w-5xl mx-auto px-4 py-12 sm:px-6 lg:px-8 flex-1">
            ...
            <div className="no-print mb-8">
              <Link
                href="/catalogs"
                className="text-xs font-bold text-slate-400 hover:text-red-500 flex items-center gap-1 w-fit"
              >
                <svg
                  width="14"
                  height="14"
            ...
            <div
              className={`rounded-3xl ${isGold ? "bg-[#f5f1e6] dark:bg-[#1c1611]" : isDark ? "bg-[#111318]" : "bg-slate-50 dark:bg-[#111318]"} border border-slate-200/60 dark:border-white/5 p-8 sm:p-16 text-center shadow-xs flex flex-col justify-center min-h-[50vh] relative overflow-hidden mb-12`}
            >
              {/* Top Accent line */}
              <div
                className={`w-24 h-0.5 mx-auto mb-8 bg-red-600 dark:bg-red-500`}
              />

              <span
                className={`text-xs font-bold uppercase tracking-[0.4em] ${isGold ? "text-[#c5a059] dark:text-[#e5c185]" : isDark ? "text-red-500 dark:text-red-400" : "text-red-700 dark:text-red-400"}`}
              >
            ...
            <div className="max-w-2xl mx-auto text-center mb-20 px-4">
              <span
                className={`text-[10px] font-bold uppercase tracking-widest ${isGold ? "text-[#c5a059] dark:text-[#e5c185]" : isDark ? "text-red-500 dark:text-red-400" : "text-red-700 dark:text-red-400"}`}
              >
              ...
              <p className="mt-4 text-sm text-slate-500 dark:text-slate-400 leading-relaxed italic">
                Welcome to this curated selection of PTC Furnitures, designed to
                elevate daily spaces into environments of beauty, comfort, and
                deep-seated structure. Each piece is constructed with finest
                premium lumber, engineered for geometric stability, and tailored
                to architectural precision. Let these designs spark your
                upcoming decor aspirations.
              </p>
              <div
                className={`w-12 h-px mx-auto mt-8 bg-slate-200 dark:bg-white/10`}
              />
            </div>
                ...
                return (
                  <div key={product.id}>
                    ...
                    >
                      {/* Product Image */}
                      <div
                        className={`relative h-72 sm:h-96 w-full rounded-2xl overflow-hidden bg-slate-100 dark:bg-white/5 border border-slate-200/40 dark:border-white/5 flex items-center justify-center ${
            ...
            <div
              className={`mt-20 rounded-3xl ${isGold ? "bg-white dark:bg-stone-900/50 border border-[#d4af37]/20 dark:border-[#d4af37]/10" : isDark ? "bg-[#111318] border border-white/5" : "bg-slate-50 dark:bg-white/5 border border-slate-200/50 dark:border-white/5"} p-8 sm:p-12 text-center shadow-xs border border-dashed`}
            >
              <span
                className={`text-[10px] font-bold uppercase tracking-widest ${isGold ? "text-[#c5a059] dark:text-[#e5c185]" : isDark ? "text-red-500 dark:text-red-400" : "text-red-700 dark:text-red-400"}`}
              >
              ...
              <h3 className="text-2xl font-bold mt-2 text-slate-900 dark:text-slate-100">
                Interested in Curated Suites?
              </h3>
              <p className="text-sm text-slate-500 dark:text-slate-400 mt-3 max-w-md mx-auto leading-relaxed">
                Contact our showroom desks. Specify this catalog portfolio title
                when booking design consultations.
              </p>
