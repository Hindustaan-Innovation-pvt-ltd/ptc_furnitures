...
export async function GET() {
  try {
    ...
    const entries = await BankingDetailsModel.find()
      .sort({ createdAt: 1 })
      .lean();
...
export async function PATCH(request: Request) {
  try {
    ...
    if (body.label !== undefined)
      update.label = body.label?.trim() || "Bank Account";
    if (body.isActive !== undefined) update.isActive = body.isActive;
    if (body.accountHolderName !== undefined)
      update.accountHolderName = body.accountHolderName?.trim() ?? "";
    if (body.bankName !== undefined)
      update.bankName = body.bankName?.trim() ?? "";
    if (body.accountNumber !== undefined)
      update.accountNumber = body.accountNumber?.trim() ?? "";
    if (body.ifscCode !== undefined)
      update.ifscCode = body.ifscCode?.trim().toUpperCase() ?? "";
    if (body.accountType !== undefined)
      update.accountType = body.accountType?.trim() ?? "Current";
    if (body.branchName !== undefined)
      update.branchName = body.branchName?.trim() || undefined;
    if (body.upiId !== undefined)
      update.upiId = body.upiId?.trim() || undefined;
    if (body.upiName !== undefined)
      update.upiName = body.upiName?.trim() || undefined;
    if (body.qrImage !== undefined) update.qrImage = body.qrImage || undefined;
    if (body.notes !== undefined)
      update.notes = body.notes?.trim() || undefined;

    const updated = await BankingDetailsModel.findByIdAndUpdate(
      id,
      { $set: update },
      { new: true },
    ).lean();
