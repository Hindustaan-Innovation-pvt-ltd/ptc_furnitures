...
export default function AdminLeadsPage() {
  ...
  async function handleDeleteLead(id: string) {
    if (
      !window.confirm(
        "Are you sure you want to delete this dealer lead? This cannot be undone.",
      )
    )
      return;
    setActionLoadingId(id);
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-slate-100 dark:border-white/5 pb-6 mb-6">
            <div>
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                Filter by partnership metrics, perform status transition checks,
                and review dealer city logs.
              </p>
            </div>
          ...
          {loading ? (
            <div className="flex flex-col items-center justify-center py-20 text-slate-400">
              ...
              <p className="text-xs font-medium uppercase tracking-wider">
                Synchronizing dealer lead logs...
              </p>
            </div>
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center text-slate-400">
              ...
              <p className="text-xs text-slate-400 mt-1">
                Try modifying your keyword searches or active filters.
              </p>
            </div>
          ) : (
                            ...
                            <div className="inline-flex items-center justify-center">
                              <Select
                                ...
                                onValueChange={(value) =>
                                  handleStatusChange(
                                    lead.id,
                                    value as DealerLead["status"],
                                  )
                                }
                              >
                                <SelectTrigger
                                  size="sm"
                                  className={`px-2.5 py-1 rounded-full border text-[10px] font-bold focus:outline-none transition-colors cursor-pointer ${getStatusBadge(lead.status)}`}
                                >
                                  <SelectValue />
                                </SelectTrigger>
                                <SelectContent>
                  ...
                  <div className="rounded-2xl border border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-[#111318]/50 p-4.5 space-y-3.5">
                    ...
                    <div className="flex justify-between text-xs">
                      ...
                      <span className="text-slate-900 dark:text-slate-100 font-bold inline-flex items-center gap-1">
                        <MapPin className="size-3 text-red-600 shrink-0" />{" "}
                        {selectedLead.city}
                      </span>
                    </div>
                    <div className="flex justify-between text-xs">
                    ...
                    <div className="flex justify-between items-center text-xs">
                      ...
                      <span
                        className={`px-2.5 py-0.5 rounded-full border text-[10px] font-bold ${getStatusBadge(selectedLead.status)}`}
                      >
                        {selectedLead.status}
                      </span>
                    </div>
                    ...
                    {selectedLead.whatsappSentAt && (
                      <div className="flex justify-between text-xs">
                        ...
                        <span className="text-slate-600 dark:text-slate-300 font-medium">
                          {new Date(selectedLead.whatsappSentAt).toLocaleString(
                            undefined,
...
Thank you for submitting your partnership request. We will review your details shortly.`;
                    ...
                    const cleanPhone = selectedLead.phone.replace(
                      /[^0-9]/g,
                      "",
                    );
                    ...
                    try {
                      ...
                      if (response.ok && data.success) {
                        setResendStatus({
                          success: true,
                          message:
                            "Opened in WhatsApp (wa.me) & Status logged!",
                        });
                        // Update local leads list
                        setLeads((prev) =>
                          prev.map((l) =>
                            l.id === selectedLead.id ? data.lead : l,
                          ),
                        );
