...
import { Star, Search, Trash2, CheckCircle, AlertCircle, MessageSquare, ShieldAlert, Award } from "lucide-react";
...
export default function AdminReviewsPage() {
  ...
  async function handleStatusChange(id: string, newStatus: ProductReview["status"]) {
  ...
  async function handleDeleteReview(id: string) {
    if (!window.confirm("Are you sure you want to delete this customer review? This cannot be undone.")) return;
    setActionLoadingId(id);
    try {
        ...
        <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="rounded-2xl border border-slate-200/60 dark:border-white/5 bg-white dark:bg-[#111318] p-5 shadow-xs flex items-center justify-between">
            <div>
              ...
              <h3 className="text-3xl font-extrabold mt-1.5 text-slate-900 dark:text-slate-100">{totalReviews}</h3>
              <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-1">Submitted ratings</p>
            </div>
          ...
          <div className="rounded-2xl border border-slate-200/60 dark:border-white/5 bg-white dark:bg-[#111318] p-5 shadow-xs flex items-center justify-between">
            <div>
              ...
              <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-1">Overall sentiment score</p>
            </div>
          ...
          <div className="rounded-2xl border border-slate-200/60 dark:border-white/5 bg-white dark:bg-[#111318] p-5 shadow-xs flex items-center justify-between">
            <div>
              <p className="text-xs font-bold uppercase tracking-wider text-slate-400">Approved Reviews</p>
              <h3 className="text-3xl font-extrabold mt-1.5 text-slate-900 dark:text-slate-100">{approvedReviews}</h3>
              <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-1">Visible on products</p>
            </div>
          ...
          <div className="rounded-2xl border border-slate-200/60 dark:border-white/5 bg-white dark:bg-[#111318] p-5 shadow-xs flex items-center justify-between">
            <div>
              <p className="text-xs font-bold uppercase tracking-wider text-slate-400">Flagged Spam</p>
              <h3 className="text-3xl font-extrabold mt-1.5 text-slate-900 dark:text-slate-100">{rejectedReviews}</h3>
              <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-1">Filtered from public</p>
            </div>
        ...
        <div className="rounded-3xl border border-slate-200/60 dark:border-white/5 bg-white dark:bg-[#111318] p-6 shadow-xs">
          ...
          {loading ? (
            <div className="flex flex-col items-center justify-center py-20 text-slate-400">
              ...
              <p className="text-xs font-medium uppercase tracking-wider">Loading product reviews...</p>
            </div>
          ) : filteredReviews.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-center text-slate-400">
              <AlertCircle className="size-12 stroke-[1.2] mb-3 text-slate-300 dark:text-slate-700" />
              <p className="text-sm font-bold text-slate-800 dark:text-slate-200">No reviews found</p>
              ...
              <table className="w-full border-collapse text-left text-xs">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-white/5">
                  {filteredReviews.map((review) => {
                    ...
                    return (
                      <tr key={review.id} className="hover:bg-slate-50/50 dark:hover:bg-white/5 transition-colors group">
                        {/* ID */}
                        ...
                        <td className="py-4 px-4 text-center">
                          <div className="flex items-center justify-center gap-0.5 text-amber-500">
                            ...
                            {Array.from({ length: 5 - review.rating }).map((_, i) => (
                              <Star key={i} className="size-3 text-slate-200 dark:text-slate-800" />
                            ))}
                          </div>
                        </td>

                        {/* Text */}
                        <td className="py-4 px-4 max-w-70 font-medium text-slate-600 dark:text-slate-300 leading-relaxed truncate" title={review.text}>
                          {review.text}
                        </td>
                        ...
                        <td className="py-4 px-4 text-center">
                          {isActionLoading ? (
                            ...
                            <div className="inline-flex items-center justify-center">
                              <Select
                                value={review.status}
                                onValueChange={(value) => handleStatusChange(review.id, value as ProductReview["status"])}
                              >
                                <SelectTrigger size="sm" className={`px-2.5 py-1 rounded-full border text-[10px] font-bold focus:outline-none transition-colors cursor-pointer ${getStatusBadge(review.status)}`}>
                                  <SelectValue />
                                </SelectTrigger>
                                <SelectContent>
                                  <SelectItem value="approved">Approved</SelectItem>
                                  <SelectItem value="pending">Pending</SelectItem>
                                  <SelectItem value="rejected">Spam Flagged</SelectItem>
                                </SelectContent>
                              </Select>
                            </div>
                          )}
