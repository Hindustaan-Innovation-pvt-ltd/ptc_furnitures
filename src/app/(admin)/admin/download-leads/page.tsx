...
import { Download, Phone, Search, Trash2, User, FileText, ImageIcon, Printer, RefreshCw } from "lucide-react";
...
function actionLabel(action: string) {
  switch (action) {
    case "image_download": return "Image Download";
    case "catalog_download": return "Catalog Download";
    case "catalog_print": return "Catalog Print";
...
function ActionBadge({ action }: { action: string }) {
  ...
  if (action === "image_download")
    return (
      <span className={`${base} bg-blue-50 text-blue-700 border-blue-200 dark:bg-blue-950/20 dark:text-blue-400 dark:border-blue-900/30`}>
        <ImageIcon className="size-2.5" /> Image Download
      </span>
    );
  if (action === "catalog_download")
    return (
      <span className={`${base} bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-950/20 dark:text-amber-400 dark:border-amber-900/30`}>
        <FileText className="size-2.5" /> Catalog PDF
      </span>
    );
  if (action === "catalog_print")
    return (
      <span className={`${base} bg-purple-50 text-purple-700 border-purple-200 dark:bg-purple-950/20 dark:text-purple-400 dark:border-purple-900/30`}>
        <Printer className="size-2.5" /> Print / Save
      </span>
    );
  return (
    <span className={`${base} bg-slate-50 text-slate-500 border-slate-200 dark:bg-white/5 dark:text-slate-400 dark:border-white/10`}>
      {action}
    </span>
  );
}
...
export default function AdminDownloadLeadsPage() {
  ...
  async function handleDelete(id: string) {
    if (!window.confirm("Delete this lead entry? This cannot be undone.")) return;
    setDeletingId(id);
    try {
  ...
  const catalogLeads = leads.filter((l) => l.action === "catalog_download").length;
  ...
  const filteredLeads = leads.filter((lead) => {
    ...
    const matchesAction = actionFilter === "all" || lead.action === actionFilter;
      ...
      <div className="grid gap-8 animate-scale-up">
        ...
        <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <div className="rounded-2xl border border-slate-200/60 dark:border-white/5 bg-white dark:bg-[#111318] p-5 shadow-xs flex items-center justify-between">
            <div>
              ...
              <p className="text-[10px] text-slate-400 mt-1">All captured entries</p>
            </div>
          ...
          <div className="rounded-2xl border border-slate-200/60 dark:border-white/5 bg-white dark:bg-[#111318] p-5 shadow-xs flex items-center justify-between">
            <div>
              <p className="text-xs font-bold uppercase tracking-wider text-slate-400">Image Downloads</p>
              <h3 className="text-3xl font-extrabold mt-1.5 text-slate-900 dark:text-slate-100">{imageLeads}</h3>
              <p className="text-[10px] text-slate-400 mt-1">Product image requests</p>
            </div>
          ...
          <div className="rounded-2xl border border-slate-200/60 dark:border-white/5 bg-white dark:bg-[#111318] p-5 shadow-xs flex items-center justify-between">
            <div>
              <p className="text-xs font-bold uppercase tracking-wider text-slate-400">Catalog Downloads</p>
              <h3 className="text-3xl font-extrabold mt-1.5 text-slate-900 dark:text-slate-100">{catalogLeads}</h3>
              <p className="text-[10px] text-slate-400 mt-1">PDF brochure requests</p>
            </div>
          ...
          <div className="rounded-2xl border border-slate-200/60 dark:border-white/5 bg-white dark:bg-[#111318] p-5 shadow-xs flex items-center justify-between">
            <div>
              <p className="text-xs font-bold uppercase tracking-wider text-slate-400">Print / Save</p>
              <h3 className="text-3xl font-extrabold mt-1.5 text-slate-900 dark:text-slate-100">{printLeads}</h3>
              <p className="text-[10px] text-slate-400 mt-1">Print-to-PDF actions</p>
            </div>
        ...
        <div className="rounded-3xl border border-slate-200/60 dark:border-white/5 bg-white dark:bg-[#111318] p-6 shadow-xs">
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-slate-100 dark:border-white/5 pb-6 mb-6">
            <div>
              <h2 className="text-xl font-bold tracking-tight text-slate-900 dark:text-slate-100">Lead Capture Log</h2>
              <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                Visitors who submitted their name &amp; mobile before downloading.
              </p>
            </div>
            <button
              onClick={fetchLeads}
          ...
          {loading ? (
            <div className="flex flex-col items-center justify-center py-20 text-slate-400">
              ...
              <p className="text-xs font-medium uppercase tracking-wider">Loading download leads...</p>
            </div>
          ) : filteredLeads.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-center text-slate-400">
              <Download className="size-12 stroke-[1.2] mb-3 text-slate-300 dark:text-slate-700" />
              <p className="text-sm font-bold text-slate-800 dark:text-slate-200">No leads yet</p>
              <p className="text-xs text-slate-400 mt-1">Download leads will appear here once visitors submit their details.</p>
            </div>
          ) : (
                      ...
                      <td className="py-4 px-4 max-w-[200px]">
                        {lead.productName ? (
                          ...
                          <span className="text-slate-400 italic text-[10px] truncate block">
                            {lead.catalogUrl.split("/").pop() || lead.catalogUrl}
                          </span>
                        ) : (
      ...
      {selectedLead && (
        <div className="fixed inset-0 z-50 flex justify-end bg-black/60 backdrop-blur-xs animate-fade-in">
          ...
          <div className="relative w-full max-w-md h-full bg-white dark:bg-[#0c0d12] shadow-2xl border-l border-slate-200/60 dark:border-white/5 p-8 flex flex-col gap-6 overflow-y-auto z-50 animate-slide-in-right">
            ...
            {/* Profile card */}
            <div>
              <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">Contact Details</h4>
            ...
            {/* Action card */}
            <div>
              <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">Download Activity</h4>
              <div className="rounded-2xl border border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-[#111318]/50 p-4 space-y-3.5">
                ...
                {selectedLead.productId && (
                  <div className="flex justify-between text-xs">
                    ...
                    <span className="font-mono text-slate-500 text-[10px]">{selectedLead.productId}</span>
                  </div>
                )}
            ...
            <div className="mt-auto pt-5 border-t border-slate-100 dark:border-white/5 grid grid-cols-2 gap-3">
              ...
              >
                <svg viewBox="0 0 24 24" className="size-3.5 fill-[#25D366]" xmlns="http://www.w3.org/2000/svg">
